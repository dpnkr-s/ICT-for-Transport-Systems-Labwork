count = db.PermanntBookings.count();
db.PermanentBookings.find().sort( {"duration":1} ).skip(count / 2 - 1).limit(1);

db.PermanentBookings.aggregate([
{
    $match:{city:"Milano"}
},
{
    $project:{
    duration:{$divide:[{$subtract:["$final_time", "$init_time"]},60]},
    distance_lat:{$subtract: ["$init_lat", "$final_lat"]},
    distance_lon:{$subtract: ["$init_lon", "$final_lon"]},
    day:  {$subtract :["$init_time",{ $mod:["$init_time", 3600*24]}]},
    hour:{$subtract:["$init_time", {$mod:["$init_time", 3600]}]}
            }
},
{
    $match:{
        $and:[
            {distance_lat:{$ne:0}},
            {distance_lon:{$ne:0}},
            {duration:{$gt:1, $lt:180}}
            ]
       }
},
{
    $project:{
        duration:1,
        day:1,
        hour:1,
        hourOfDay:{$subtract:["$hour", "$day"]}
    }
},
{
        $group:{
            _id: "$day",
            tot:{$sum:1},
            avg_duration: {$avg:"$duration"}
        }},
    {
        $sort:{_id:1}
    }]).forEach(function(o){
       
        print(new Date(o._id)+ " " + o.tot, o.avg_duration)
    }
)